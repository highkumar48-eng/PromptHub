...
export function monetizationProgress({ publishedPrompts, measuredVisitors, storedStatus, creatorSharePercent = 50 }) {
  const requirements = { prompts: 10, visitors: 5000 };
  const eligible = publishedPrompts >= requirements.prompts && measuredVisitors >= requirements.visitors;
  const status = storedStatus === 'monetized' || storedStatus === 'paused' ? storedStatus : eligible ? 'eligible_for_review' : 'not_eligible';
  return { status, requirements, publishedPrompts, measuredVisitors, creatorSharePercent, eligible };
}
