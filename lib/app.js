...
import { normalizeKeyword, validatePrompt, monetizationProgress } from './domain.js';
...
async function countPublicVisitor(db, request, creatorId, localPreview = false) {
  ...
  // Retain a short rolling window only; the raw, pseudonymous visitor tokens are never shown to creators.
  await db.run("DELETE FROM creator_daily_visitors WHERE metric_date < date('now','-90 days')");
  await db.run("DELETE FROM creator_daily_metrics WHERE metric_date < date('now','-90 days')");
  return setCookie;
}
async function creatorMonetization(db, creatorId) {
  const [published, metrics, stored] = await Promise.all([
    db.one("SELECT COUNT(*) AS count FROM prompts WHERE creator_id=? AND status='published'", creatorId),
    db.one("SELECT COALESCE(SUM(unique_visitors),0) AS count FROM creator_daily_metrics WHERE creator_id=? AND metric_date >= date('now','-29 days')", creatorId),
  ...
  return monetizationProgress({ publishedPrompts: Number(published.count), measuredVisitors: Number(metrics.count), storedStatus: stored?.status, creatorSharePercent: stored?.creator_share_percent || 50, activatedAt: stored?.activated_at, pausedReason: stored?.paused_reason });
}
