...
export async function handleRequest(request, env) {
  ...
  try {
    ...
    if (pathname === '/creator' && user) creator = await db.one('SELECT * FROM creators WHERE user_id = ?', user.id);
    ...
    if (pathname === '/join') return redirect('/creator');
    if (pathname === '/creator') {
      if (!user) return page('creator-portal', base());
      return creator ? redirect('/dashboard') : page('join', { ...base(), form: {}, error: '' });
    }
